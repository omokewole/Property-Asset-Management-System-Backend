...
export async function handleVerifyUser(req, res) {
	try {
		...
		await sendConfirmationMail(result.user);
...
export async function handleUserPropertyPerformanceReport(req, res) {
	try {
		const userId = req.user._id;

		const reports = await userPropertyPerformanceReport(userId);
		...
		res
			...
			.json(
				responseModel(
					true,
					`User Property performace reports fetched successfully`,
					reports
				)
			);
	} catch (error) {
